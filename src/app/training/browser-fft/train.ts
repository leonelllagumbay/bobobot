...
export function createBrowserFFTModel(
    inputShape: tf.Shape, numClasses: number): any {
...
async function main() {
  ...
  const trainDataset: any = loadDataset(trainDataDir);
  const vocab = trainDataset.getVocabulary();
  (tf.util as any).assert(
      vocab.length > 1,
  ...
  if (fs.existsSync(path.join(args.dataPath, 'test'))) {
    ...
    const {xs: testXs, ys: testYs} =
        (testDataset as any).getSpectrogramsAsTensors(null, {shuffle: true});
